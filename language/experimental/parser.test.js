...
describe("parser – function call examples from syntax.md", () => {
  ...
  // 23. foo -verbose bar x, -switch:on  # valid, outer args comma-separated
  it("parses 'foo -verbose bar x, -switch:on' with verbose = bar(x), outer args by comma", () => {
    expect(expr("foo -verbose bar x, -switch:on")).toEqual({
      type: "CallExpr",
      callee: { type: "Identifier", name: "foo" },
      args: [
  ...
  // 24. foo -verbose (bar x, baz y, z) -switch:on
  it("parses 'foo -verbose (bar x, baz y, z) -switch:on' with nested comma-separated bar(...) call", () => {
    expect(expr("foo -verbose (bar x, baz y, z) -switch:on")).toEqual({
      type: "CallExpr",
      callee: { type: "Identifier", name: "foo" },
      args: [
        {
  ...
  // 25. foo -verbose bar x, y, -switch:on  # invalid
  it("rejects 'foo -verbose bar x, y, -switch:on' (nested call with commas but not parenthesized)", () => {
    expect(() => parse("foo -verbose bar x, y, -switch:on")).toThrow(
      SyntaxError,
    );
  });

  // 26. foo -verbose (bar x, y), -switch:on  # valid
  it("parses 'foo -verbose (bar x, y), -switch:on' as foo(verbose = bar(x, y), enum)", () => {
    expect(expr("foo -verbose (bar x, y), -switch:on")).toEqual({
      type: "CallExpr",
      callee: { type: "Identifier", name: "foo" },
      args: [
        {
